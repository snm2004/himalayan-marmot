...
const Chatbot: React.FC = () => {
  ...
  const handleSend = async (text: string = input) => {
    ...
    try {
      ...
      // Constructing a detailed context from constants for the AI
      const tourSummary = TOUR_PACKAGES.map(p => `- ${p.title} (${p.duration}): Starts at ${p.startingPrice}. Highlights: ${p.highlights.join(', ')}`).join('\n');
      const policySummary = CANCELLATION_POLICIES.map(p => `${p.period}: ${p.deduction}`).join('\n');

      const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: text,
        config: {
          systemInstruction: `You are the Official Digital Guide for ${COMPANY_NAME}. 
          
          STRICT LIMITATION RULE:
          - You ONLY answer questions based on the website content of ${COMPANY_NAME}.
          - Topics you CAN discuss: Tour packages (Pangong, Turtuk, Umling La, Manali-Leh), bike fleet (RE Himalayan BS6), pricing, inclusions/exclusions, cancellation policies, bank details, safety protocols (oxygen, backup car), and company location/contact.
          - If a user asks ANYTHING ELSE (jokes, global news, coding, personal advice, unrelated general knowledge), you MUST decline.

          PREDEFINED REFUSAL MESSAGE:
          "Julley! I am specifically trained to assist with Himalayan Marmot bike expeditions and Ladakh travel logistics. I cannot answer questions on other topics. Please ask me about our tours, bikes, or booking policies!"

          WEBSITE CONTEXT:
          - Company: ${COMPANY_NAME}
          - Founder: ${FOUNDER_NAME}
          - Address: ${ADDRESS_MAIN}
          - Packages: ${tourSummary}
          - Inclusions: ${COMMON_INCLUSIONS.join(', ')}
          - Exclusions: ${COMMON_EXCLUSIONS.join(', ')}
          - Cancellation Policy: ${policySummary}
          - Bank: ${BANK_DETAILS.bank}, A/C: ${BANK_DETAILS.accountNo}, IFSC: ${BANK_DETAILS.ifsc}
          - Fleet: ${FLEET.map(f => f.name).join(', ')}

          TONE & STYLE:
          - Professional yet adventurous. 
          - Use 'Julley' (Ladakhi greeting) naturally. 
          - Keep answers concise and factual. 
          - Never hallucinate prices or dates not listed.`
        }
      });

      const botText = response.text || "Julley! I encountered a small roadblock. Could you please rephrase your question?";
      setMessages(prev => [...prev, { role: 'bot', text: botText }]);
    } catch (error) {
      console.error("Chatbot Error:", error);
      setMessages(prev => [...prev, { role: 'bot', text: "Julley! My connection to the mountains is weak. Please reach us directly on WhatsApp for assistance!" }]);
    } finally {
      setIsLoading(false);
    }
  };
             ...
             {showGreeting && (
               <div className="absolute bottom-20 right-0 bg-white p-4 rounded-3xl shadow-2xl border border-slate-100 w-48 animate-bounce transition-all">
                  <p className="text-xs font-bold text-mountain-blue leading-snug">
                    Julley! Ask me anything about our 2026 bike tours! 🏔️
                  </p>
